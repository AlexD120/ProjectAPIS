...
def test_api_positive(api_request, set_up):
    ...
    assert all(item in response.json().items() for item in body_requests.request_put_update.items())
...
def test_web(driver):
    ...
    assert all(item in value_cv.items() for item in body_requests.request_put_update.items())
