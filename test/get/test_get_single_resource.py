...
def test_api_positive(api_request, set_up):
    ...
    body = body_response.response_get_single_resource
...
def test_api_negative(api_request, set_up):
    url = "https://reqres.in/appium/nonexistent-endpoint"
    response = api_request(url)
    assert response.status_code == 404


def test_api_negative_2(api_request, set_up):
    url = "https://reqres.in/api/unknown/2?param=value"
    response = api_request(url)
    assert response.status_code == 200
    not_word = Base(conftest.driver)
    not_word.check_word_not_in_json(response, "Я быстро учусь")
    print("test_api_negative_2 - PASSED")


# WEB тесты
def test_web(driver):
    button_locator = "//li[@data-id='unknown-single']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/unknown/2"
    status = "200"
    ...
    assert value_cv == body_response.response_get_single_resource
