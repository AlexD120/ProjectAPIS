...
def test_api_positive(api_request, set_up):
    ...
    body = body_response.response_get_single_user_not_found

    assert response.status_code == 404
    assert response.json() == body  # Проверяем, что тело ответа соответствует ожидаемому
    print("test_api_positive - PASSED")

# Негативные API тесты
def test_api_negative(api_request, set_up):
    url = "https://reqres.in/apis/users?page=invalid"
    response = api_request(url)

    assert response.status_code == 404
    print("test_api_negative - PASSED")
...
def test_api_negative_2(api_request, set_up):
    ...
    not_word.check_word_not_in_json(response, "Хочу к вам в команду!")
    print("test_api_negative_2 - PASSED")


def test_web(driver):
    button_locator = "//li[@data-id='users-single-not-found']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/users/23"
    status = "404"
    ...
    assert value_cv == body_response.response_get_single_user_not_found
