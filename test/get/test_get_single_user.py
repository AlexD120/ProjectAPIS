...
def test_api_positive(api_request):
    ...
    body = body_response.response_get_single_users
...
def test_api_negative(api_request, set_up):
    url = "https://reqres.in/api/users/invalid_id"
    response = api_request(url)
...
def test_api_negative_2(api_request, set_up):
    ...
    not_word.check_word_not_in_json(response, "PlanB")
    print("test_api_negative_2 - PASSED")

def test_web(driver):
    button_locator = "//li[@data-id='users-single']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/users/2"
    status = "200"
    ...
    assert value_cv == body_response.response_get_single_users
