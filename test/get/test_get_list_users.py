import pytest
import body_requests.body_requests
import conftest
...
def test_api_positive(api_request, set_up):
    ...
    body = body_response.response_get_list_users
...
# WEB тесты
def test_web(driver):
    button_locator = "//li[@data-id='users']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/users?page=2"
    status = "200"
    ...
    assert value_cv == body_response.response_get_list_users
