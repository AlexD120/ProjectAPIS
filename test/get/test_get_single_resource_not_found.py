...
def test_api_positive(api_request, set_up):
    ...
    body = body_response.response_get_single_resource_not_found
...
def test_api_negative(api_request, set_up):
    url = "https://reqres.in/apius/nonexistent-endpoint"
    response = api_request(url)
...
def test_api_negative_2(api_request, set_up):
    ...
    not_word.check_word_not_in_json(response, "Буду очень стараться")
    print("test_api_negative_2 - PASSED")


def test_web(driver):
    button_locator = "//li[@data-id='unknown-single-not-found']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/unknown/23"
    status = "404"
    ...
    assert value_cv == body_response.response_get_single_resource_not_found
