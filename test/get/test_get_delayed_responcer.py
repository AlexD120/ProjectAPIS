...
def test_api_positive(api_request):
    url = "https://reqres.in/api/users?delay=3"
    response = api_request(url)

    body = body_response.response_get_delayed_response
...
def test_api_negative(api_request, set_up):
    url = "https://reqres.in/apia/users?delay=3"
    response = api_request(url)
...
def test_api_negative_2(api_request, set_up):
    url = "https://reqres.in/api/invalid_endpoint"
    response = api_request(url)

    assert response.status_code == 200
    not_word = Base(conftest.driver)
    not_word.check_word_not_in_json(response, "Спасибо за внимание!")
    print("test_api_negative_2 - PASSED")

def test_web(driver):
    button_locator = "//li[@data-id='delay']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/users?delay=3"
    status = "200"
    ...
    assert value_cv == body_response.response_get_delayed_response
