...
def test_api_negative_2(api_request, set_up):
    ...
    not_word.check_word_not_in_json(response, "Отвественно подхожу к работе")
    print("test_api_negative_2 - PASSED")


# WEB тесты
def test_web(driver):
    button_locator = "//li[@data-id='post']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/users"
    status = "201"
    ...
    assert all(item in value_cv.items() for item in body_requests.request_body_post_create.items())
