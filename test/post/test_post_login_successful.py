...
def test_api_positive(api_request, set_up):
    ...
    data = body_requests.request_post_login_successful
    response = api_request(url, method="POST", data=data)

    assert response.status_code == 200
    assert all(item in response.json().items() for item in body_response.response_post_login_successful.items())
...
def test_api_negative_invalid_url(api_request):
    ...
    not_word.check_word_not_in_json(response, "Хочу заниматься любимым делом")
    print("test_api_negative_2 - PASSED")


def test_web(driver):
    button_locator = "//li[@data-id='login-successful']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/login"
    status = "200"
    ...
    assert all(item in value_cv.items() for item in body_response.response_post_login_successful.items())
