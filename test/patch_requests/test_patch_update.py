...
def test_api_positive(api_request, set_up):
    ...
    data = body_requests.request_put_update
    response = api_request(url, method="PATCH", data=data)

    assert response.status_code == 200
    assert all(item in response.json().items() for item in body_requests.request_patch_update.items())
...
def test_web(driver):
    button_locator = "//li[@data-id='put']"  #ЛОКАТОР ДЛЯ ID АЖДОГО ТЕСТА
    key_url = "/api/users/2"
    status = "200"
    ...
    assert all(item in value_cv.items() for item in body_requests.request_patch_update.items())
